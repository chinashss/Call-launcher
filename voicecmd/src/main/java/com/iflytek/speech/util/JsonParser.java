...
//				}
	...
	public static int parseCommandResult( String json ) {
		int cmd_id = -1;
		StringBuffer ret = new StringBuffer();
		try {
			JSONTokener tokener = new JSONTokener(json);
			...
			for (int i = 0; i < words.length(); i++) {
				JSONObject wsItem = words.getJSONObject(i);
				JSONArray items = wsItem.getJSONArray("cw");
				if ("<name>".equals(wsItem.getString("slot"))) {
					// 可能会有多个联系人供选择，用中括号括起来，这些候选项具有相同的置信度
					ret.append("【");
					for (int j = 0; j < items.length(); j++) {
						JSONObject obj = items.getJSONObject(j);
						if (obj.getString("w").contains("nomatch")) {
							ret.append("没有匹配结果.");
							Log.d( VoiceCmdEngine.TAG, ret.toString());
							return -1;
						}
						ret.append(obj.getString("w")).append("|");
						cmd_id = obj.getInt("id");
					}
					ret.setCharAt(ret.length() - 1, '】');
				} else {
					//本地多候选按照置信度高低排序，一般选取第一个结果即可
	...
	public static int parseCommandResult2( String json ) {
		int cmd_id = -1;
		int cmd_score = 0;
		StringBuffer ret = new StringBuffer();
		try {
